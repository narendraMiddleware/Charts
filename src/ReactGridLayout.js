...
const ReactGridLayout = ({children}) => {
  ...
  const loadData = (url) => {
    const response = fetch('https://mocki.io/v1/bf278006-a3a7-4305-b36b-7b7ef8b61bd2');
  ...
  const CustomChart = (p) => {
    return <LineChart data={typeof p.json !== "undefined" ? p.json : data} />
  };

  const addChartOnBtnClick = json => {
      console.log(json,"Json")
    setChartList(chartList.concat(<CustomChart json={json} key={chartList.length} />));
  };
