...
export default function HeatMap(props) {
        ...
        let [width, setWidth] = useState(450);
        ...
        useEffect(() => {
          ...
          .attr("viewBox", "0 0 1800 100") //1200 100

          //allows text to be placed on or near legend boxes by appending both the text and boxes to this parent variable
          let rects = legend.selectAll("rect")
            .data(colorScheme)
            .enter();

          let legendBoxHeight = 30;
          let legendBoxWidth = 50;
          let legendValues = ["-7", "-5", "-3", "-2", "0", "1", "3", "5"];

          //add color boxes to legend
          rects.append("rect")
              .attr("x", function(d, i) {
            return margins.left + (legendBoxWidth * i)
          })
              .attr("y", 45)
          ...
          //add labels for the boxes on the legend
          rects.append("text")
            .attr("fill", "black")
            .attr("x", function(d, i) {
            //this statement centers the legend values accounting for negative sign
            if (legendValues.indexOf(i) >= 4) {
              return margins.left + (legendBoxWidth / 5) + (legendBoxWidth * i);
            } else {
              return margins.left + (legendBoxWidth / 3) + (legendBoxWidth * i);
            }
          })
            .attr("y", 100)
            .style("font-size", "20")
      ...
      return(
        ...
        <div id="main">
        <svg className={props.classnameCustom} id="chart"></svg>
        <svg id="legend"></svg>
          <div id="tooltip">
              <div id="container">
